...
class BunkerEnv(MujocoEnv):
    ...
    def _initialize_velocity_controller(self):
        ...
        # BunkerVelocityController.__call__ chỉ dùng w_track & r_wheel để suy ra
        # (w_left, w_right) từ (v, omega) theo công thức vi sai chuẩn -- công
        # thức này KHÔNG phụ thuộc số bánh mỗi bên, nên class cũ tái dùng được
        # nguyên vẹn cho robot mobile 2-bánh, chỉ đổi tên body/actuator.
        ...
        id_geom = self.model.geom("left_wheel_geom").id
    ...
    def _set_qpos_pose(self, x: float, y: float, theta: float,
                       height: float = 0.005) -> None:
        """
        Đặt freejoint qpos = [x, y, z, qw, qx, qy, qz].

        height=0.005: đáy bánh chạm sàn khi mobile_base origin ở z=0
        (wheel center z = 0.085 m = wheel radius, xem mobile_body.xml).
        Chỉ chừa 5mm khe hở an toàn tránh chèn lún lúc khởi tạo.
        """
        self.data.qpos[:] = 0.0
        self.data.qpos[0] = x
        self.data.qpos[1] = y
    ...
    # 4 caster ("caster_fr/fl/rl/rr") là geom nằm trực tiếp trên body
    # "base_link", không phải body riêng, nên tự động được lọc qua tiền
    # tố "base_link" -- không cần thêm dòng riêng cho caster.
    ...
    def compute_reward(self, achieved_goal, desired_goal, info) -> np.ndarray:
        ...
        if isinstance(info, dict):
            ...
            front_pen     = float(info.get("front_warning_penalty", 0.0))
        else:
            ...
            front_pen     = np.array([x.get("front_warning_penalty", 0.0) for x in info], dtype=np.float32)
    ...
    def grid_to_world(self, gx, gy, resolution=0.1):
        x = self.xy_min[0] + (gx + 0.5) * resolution
