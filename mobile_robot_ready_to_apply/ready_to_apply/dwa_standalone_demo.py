...
----------------------------------------------------------------
Chạy robot trong MuJoCo CHỈ với DWA (không dùng SAC/RL).
"""
...
def setup_velocity_controller(model, data, verbose=True):
    ...
    id_r = model.body("right_wheel").id
    id_l = model.body("left_wheel").id
    pos_r = model.body_pos[id_r]
    pos_l = model.body_pos[id_l]
    diff  = pos_r - pos_l

    # ── auto-detect trục track width (trục có khoảng cách lớn nhất) ──
    best_axis   = int(np.argmax(np.abs(diff)))
    width_track = float(abs(diff[best_axis]))

    if verbose:
        print(f"\n[DIAG] right_wheel pos = {pos_r}")
        print(f"[DIAG] left_wheel  pos = {pos_l}")
        print(f"[DIAG] diff            = {diff}")
        print(f"[DIAG] -> Track width = {width_track:.4f} m  (truc {['X','Y','Z'][best_axis]})")

    vc.w_track = width_track

    id_geom    = model.geom("left_wheel_geom").id
    vc.r_wheel = float(model.geom_size[id_geom][0])
    if verbose:
        print(f"[DIAG] Wheel radius = {vc.r_wheel:.4f} m")

    vc.act_r = [model.actuator("right_wheel").id]
    vc.act_l = [model.actuator("left_wheel").id]

    mujoco.set_mjcb_control(vc)
    return vc


def set_robot_pose(data, x, y, theta, height=0.005):
    data.qpos[:] = 0.0
    data.qpos[0] = x
    data.qpos[1] = y
    data.qpos[2] = height
...
def parse_args():
    ...
    default_xml = os.path.join(_PROJECT_ROOT, "assets", "worlds", "mobile_empty.xml")
    ...
    # NOTE: world bounds nay la mac dinh cho world 15x15m dung chung voi
    # Bunker (xy_min=[-4,-4], xy_max=[11,11] trong gym_bunker_env.py).
    # Neu ban dung world khac kich thuoc, doi lai 2 dong duoi cho khop.
    p.add_argument("--world_min", nargs=2, type=float, default=[-4., -4.])
    p.add_argument("--world_max", nargs=2, type=float, default=[11., 11.])
