...
class BunkerEnv(MujocoEnv):
    ...
    def _initialize_velocity_controller(self):
        ...
        # MOBILE ROBOT: 1 bánh mỗi bên (không phải 3 như Bunker skid-steer).
        # argmax(|diff|) vẫn tổng quát đúng, không cần đổi logic, chỉ đổi tên.
        id_r = self.model.body("right_wheel").id
        id_l = self.model.body("left_wheel").id
        diff        = self.model.body_pos[id_r] - self.model.body_pos[id_l]
        width_track = float(np.max(np.abs(diff)))
        self.velocity_controller.w_track = width_track

        # MOBILE ROBOT: wheel geom là cylinder PRIMITIVE (không phải mesh như
        # Bunker) -> geom_size[0] là bán kính THẬT, không còn nhập nhằng như
        # scaled_bunker.stl (AABB vs mesh-vertex conflict đã gặp trước đó).
        id_geom = self.model.geom("right_wheel_geom").id
        self.velocity_controller.r_wheel = float(self.model.geom_size[id_geom][0])

        # MOBILE ROBOT: 1 actuator mỗi bên, không phải 3. Danh sách 1 phần tử
        # vẫn tương thích với vòng lặp `for idx in self.act_r` trong
        # BunkerVelocityController.__call__ -- không cần sửa class đó.
        self.velocity_controller.act_r = [self.model.actuator("right_wheel").id]
        self.velocity_controller.act_l = [self.model.actuator("left_wheel").id]

        self.sensor_frc_r = [self.model.sensor("sf_right").id]
        self.sensor_frc_l = [self.model.sensor("sf_left").id]
    ...
    def _set_qpos_pose(self, x: float, y: float, theta: float,
                       height: float = 0.10) -> None:
        # MOBILE ROBOT default resting height. mobile_body.xml's own comment
        # derives this: wheel joint z-offset = base_link_offset(0.105) +
        # wheel_joint_offset(-0.02) = 0.085 = wheel radius -> wheel bottom
        # touches floor exactly at origin z=0. 0.10 keeps a small settle
        # clearance, matching the XML's own spawn pos="-4.0 -4.0 0.10".
        # (Bunker's old default was 0.25 -- specific to its own geometry,
    ...
    # MOBILE ROBOT (2-wheel diff-drive + 4 free casters). Names come from
    # mobile_body.xml. Casters/laser/imu are geoms/bodies with
    # contype=0/conaffinity=0 in the XML (laser, imu) or plain sphere geoms
    # directly on "base_link" (casters) -- covered by the "base_link" prefix,
    # no separate entries needed for them.
    _ROBOT_BODY_PREFIXES = (
        "mobile_base", "base_link", "left_wheel", "right_wheel",
    )
