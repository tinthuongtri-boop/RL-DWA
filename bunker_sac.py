...
if __name__ == "__main__":
    ...
    # ── MAP CỐ ĐỊNH: map_1_corridor (hành lang 25×8m) ────────────────
    # Spawn: [2, 2, 0]   Goal: [23, 0, 0]  (cố định trong gym_bunker_env.py)
    MAP_XML = os.path.join(root, "..", "assets", "worlds", "map_1.xml")
    if not os.path.exists(MAP_XML):
        raise FileNotFoundError(
            f"Không tìm thấy map: {MAP_XML}\n"
            "Hãy copy map_1_corridor.xml vào thư mục assets/worlds/"
        )
    train_xml_pool = [MAP_XML]
    val_xml_pool   = [MAP_XML]   # val cùng map vì spawn/goal đã cố định

    # ── HYPERPARAM ────────────────────────────────────────
    # max_ep_steps tăng lên 1000 vì hành lang dài 21m (từ x=2 → x=23)
    # Với v_max=0.5 m/s và DWA dt=0.1s → ~420 steps lý tưởng;
    # để robot thêm thời gian học tránh 7 obstacle → 1000 bước.
    TOTAL_STEPS  = 50_000
    N_ENVS       = 10
    MAX_EP_STEPS = 1000
    ...
    # ─────────────────────────────────────────────────────
    # 🔥 CONFIG RESUME  (đặt RESUME_RUN = None để train mới từ đầu)
    # ─────────────────────────────────────────────────────
    RESUME_RUN   = None        # None → tạo run mới (vd: run_14)
