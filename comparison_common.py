...
# ══════════════════════════════════════════════════════════════════════
ENV_NAME      = "test/World_Hard"
MAX_GOAL_DIST = 12
N_EPISODES    = 1
MAX_EP_STEPS  = 1000
SEED_BASE     = 1024          # seeds = [1000, 1001, ..., 1000+N-1]
...
def run_single_episode_with_astar(env, policy_fn, seed: int) -> dict:
    ...
    if grid_path is None or len(grid_path) == 0:
        print(f"[A* ERROR] Seed {seed}: Không tìm thấy đường!")
        world_waypoints = [obs["desired_goal"].copy()]
    else:
        world_waypoints = []

        for i in range(0, len(grid_path), 10):
            gx, gy = grid_path[i]

            if not raw_env.is_cell_free(gx, gy, grid_map):
                continue

            wp_2d = raw_env.grid_to_world(gx, gy, resolution=0.1)
            world_waypoints.append(np.array([wp_2d[0], wp_2d[1], z_val]))

        row, col = grid_path[-1]
        if raw_env.is_cell_free(col, row, grid_map):
            wp_2d = raw_env.grid_to_world(col, row, resolution=0.1)
            world_waypoints.append(np.array([wp_2d[0], wp_2d[1], z_val]))
        else:
            # Tìm lùi waypoint cuối an toàn
            for row, col in reversed(grid_path[-5:]):
                if raw_env.is_cell_free(col, row, grid_map):
                    wp_2d = raw_env.grid_to_world(col, row, resolution=0.1)
                    world_waypoints.append(np.array([wp_2d[0], wp_2d[1], z_val]))
                    break
    ...
    for step in range(MAX_EP_STEPS):
        ...
        if info.get("stuck_counter", 0) > 30:
            ...
            if new_grid_path:
                ...
                for i in range(0, len(new_grid_path), 10):
                    gx, gy = new_grid_path[i]

                    if not raw_env.is_cell_free(gx, gy, new_grid_map):
                        continue

                    wp_2d = raw_env.grid_to_world(gx, gy, resolution=0.1)
                    world_waypoints.append(np.array([wp_2d[0], wp_2d[1], z_val]))

                gx, gy = new_grid_path[-1]
                wp_2d = raw_env.grid_to_world(gx, gy, resolution=0.1)
                world_waypoints.append(np.array([wp_2d[0], wp_2d[1], z_val]))
